...
start.addEventListener("click", function(evt) {
    evt.preventDefault();
    startAudio.play(); // Plays sound for 'Game Start'
    startAudio.currentTime = 0;
    startGame();
})

function startGame() { // Called when the 'Start Game' button is clicked
...
function checkOrder() { 
    if (JSON.stringify(order) != JSON.stringify(playerOrder)) { // Compares sequences of the game and player
        endGame(); // Displays 'Game Over' if sequences don't match
...
function flashOrder() { // Called on game's turn
...
function flashButtons() { // Called after sequence order is set
